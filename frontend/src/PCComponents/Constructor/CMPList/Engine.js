...
const Engine = (props) => {
    ...
    const topics = [
        {name: "Производитель", key: "manufacturer"},
        {name: "Вес (кг)", key: "weight"},
        {name: "Диаметр вала (мм)", key: "shaft_diameter"},
        {name: "Мощность (Вт)", key: "peak_power"}
    ]
    const [isFilterActive, setFilterActive] = useState(false)

    useEffect(() => {
        let data = []
        if (props.manager.max_cmp >= 3){
            data = props.manager.getFilteredItems("engine")
        }
        setData(data)
    }, [])
    ...
    const userChoice = (index) => {
        ...
        if (index === cur_config[2].active_index){ //remove item
            props.setConfig(null, 2)
        }else{
            data.forEach(item => {
                if (item.id === index){props.setConfig(item, 2)}
            })
        }
    }
                    ...
                    return <li key={index} className="example_item">
                        <div className="section_1">
                            ...
                            <button className="add" onClick={() => {userChoice(item.id)}}>
                                {(props.config[2].active_index === null)?"Добавить":((props.config[2].active_index === item.id)?"Убрать":"Заменить")}
                            </button>
                        </div>
